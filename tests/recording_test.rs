...
async fn test_advertisement_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    metadata1.insert("mpris:trackid".to_string(), Value::from("track1").try_to_owned()?);
    metadata1.insert("xesam:title".to_string(), Value::from("Song 1").try_to_owned()?);
    metadata1.insert("xesam:artist".to_string(), Value::from(vec!["Artist 1"]).try_to_owned()?);
    metadata1.insert("xesam:album".to_string(), Value::from("Album 1").try_to_owned()?);
    mock_tx.send(MockCommand::Metadata(metadata1)).await?;
    mock_tx.send(MockCommand::PlaybackStatus("Playing".to_string())).await?;
    ...
    metadata2.insert("mpris:trackid".to_string(), Value::from("track2").try_to_owned()?);
    metadata2.insert("xesam:title".to_string(), Value::from("Song 2").try_to_owned()?);
    metadata2.insert("xesam:artist".to_string(), Value::from(vec!["Artist 2"]).try_to_owned()?);
    metadata2.insert("xesam:album".to_string(), Value::from("Album 2").try_to_owned()?);
    mock_tx.send(MockCommand::Metadata(metadata2)).await?;
    ...
    metadata_ad.insert("mpris:trackid".to_string(), Value::from("/com/spotify/ad/12345").try_to_owned()?);
    metadata_ad.insert("xesam:title".to_string(), Value::from("Ad Title").try_to_owned()?);
    metadata_ad.insert("xesam:artist".to_string(), Value::from(vec![""]).try_to_owned()?);
    ...
    for _ in 0..10 {
        if song2_path.exists() { song2_exists = true; break; }
        tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
    }
    assert!(song2_exists, "Exported Song 2 should exist at {:?}", song2_path);

    // The advertisement itself should NOT be exported
    let ad_path1 = music_dir.join(" - ").join("00 - Ad Title.mp3");
    let ad_path2 = music_dir.join("Unknown Artist - Unknown Album").join("00 - Ad Title.mp3");
    ...
    metadata3.insert("mpris:trackid".to_string(), Value::from("track3").try_to_owned()?);
    metadata3.insert("xesam:title".to_string(), Value::from("Song 3").try_to_owned()?);
    metadata3.insert("xesam:artist".to_string(), Value::from(vec!["Artist 3"]).try_to_owned()?);
    metadata3.insert("xesam:album".to_string(), Value::from("Album 3").try_to_owned()?);
    mock_tx.send(MockCommand::Metadata(metadata3)).await?;
    ...
    metadata4.insert("mpris:trackid".to_string(), Value::from("track4").try_to_owned()?);
    metadata4.insert("xesam:title".to_string(), Value::from("Song 4").try_to_owned()?);
    metadata4.insert("xesam:artist".to_string(), Value::from(vec!["Artist 4"]).try_to_owned()?);
    metadata4.insert("xesam:album".to_string(), Value::from("Album 4").try_to_owned()?);
    mock_tx.send(MockCommand::Metadata(metadata4)).await?;
    ...
    for _ in 0..10 {
        if song3_path.exists() { song3_exists = true; break; }
        tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
    }
    assert!(song3_exists, "Exported Song 3 should exist at {:?}", song3_path);
