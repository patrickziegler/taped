...
async fn test_recording_lifecycle() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 4. Change track to Song 2 (should trigger waiting_for_next_track = false, BUT NOT start recording Song 2 yet because it's the FIRST track change after enabling)
    ...
    let home = std::env::var("HOME").unwrap_or_else(|_| "/root".to_string());
    let music_dir = PathBuf::from(home).join("Music").join("Spotify");
    let song1_path = music_dir.join("Unknown Artist").join("Unknown Album").join("00 - Song 1.mp3");
    assert!(!song1_path.exists(), "Song 1 should NOT be exported because it was playing when recording was enabled");

    // 5. Change track to Song 3 (should START recording Song 3)
    ...
    // 6. Change track to Song 4 (should trigger Song 3 export)
    ...
    // 8. Disable recording mode while Song 4 is recording
    ...
    // 9. Song 4 should NOT be exported
    let song4_path = music_dir.join("Unknown Artist - Unknown Album").join("00 - Song 4.mp3");
    assert!(!song4_path.exists(), "Song 4 should NOT be exported because recording was disabled during its capture");
