...
async fn test_stop_discard_behavior() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tokio::spawn(exporter_task(exporter_rx, music_dir.clone(), "{title}".to_string()));
    ...
    metadata1.insert("xesam:title".to_string(), Value::from("Initial").try_to_owned()?);
    ...
    mock_tx.send(MockCommand::PlaybackStatus("Paused".to_string())).await?;
    tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;

    // 4. Check if Song 2 was exported
    let song2_path = music_dir.join("Song 2.mp3");
    assert!(!song2_path.exists(), "Song 2 should NOT have been exported after pause");
